...
export class PostgresSpanExporter implements SpanExporter {
  ...
  private async exportSpan(span: ReadableSpan): Promise<void> {
    ...
    try {
      ...
      const userId = attributes['user.id'] as string || 'anonymous';
      const sessionId = attributes['session.id'] as string || uuidv4();

      // Handle root spans (requests) differently from child spans (tool calls)
      if (!parentSpanId) {
        // This is a root span (request)
        const requestText = attributes['request.text'] as string || '';
        const responseText = attributes['response.text'] as string || '';
        const modelName = attributes['model.name'] as string || '';
        const totalTokens = attributes['tokens.total'] as number || 0;
        const promptTokens = attributes['tokens.prompt'] as number || 0;
        const completionTokens = attributes['tokens.completion'] as number || 0;
        ...
        const toolName = attributes['tool.name'] as string || name;
        const toolInput = attributes['tool.input'] as string || '';
        const toolOutput = attributes['tool.output'] as string || '';
        const toolStatus = attributes['tool.status'] as string || status.code.toString();

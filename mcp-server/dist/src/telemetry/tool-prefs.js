 ...
 */
export async function getUserTools(req, res) {
    ...
    try {
        ...
        const toolsResult = await pool.query(`SELECT 
       ...
       FROM telemetry.tool_registry`);
        ...
        const prefsResult = await pool.query(`SELECT tool_name, is_enabled
 ...
 */
export async function updateUserToolPrefs(req, res) {
    ...
    try {
        ...
        const toolExists = await pool.query(`SELECT 1 FROM telemetry.tool_registry WHERE tool_name = $1`, [toolName]);
        if (toolExists.rowCount === 0) {
            return res.status(404).json({ error: 'Tool not found in registry' });
        }
        // Upsert user preference
        await pool.query(`INSERT INTO telemetry.user_tool_prefs (user_id, tool_name, is_enabled)
       VALUES ($1, $2, $3)
       ON CONFLICT (user_id, tool_name) 
       DO UPDATE SET is_enabled = $3, updated_at = NOW()`, [userId, toolName, isEnabled]);
 ...
 */
export async function resetUserToolPrefs(req, res) {
    const userId = req.params.userId;
    if (!userId) {
        return res.status(400).json({ error: 'User ID is required' });
    }
    try {
        await pool.query(`DELETE FROM telemetry.user_tool_prefs WHERE user_id = $1`, [userId]);
        return res.status(200).json({
            success: true,
 ...
 */
export async function isToolEnabled(userId, toolName) {
    try {
        // First check user preferences
        const userPrefResult = await pool.query(`SELECT is_enabled FROM telemetry.user_tool_prefs 
       WHERE user_id = $1 AND tool_name = $2`, [userId, toolName]);
        ...
        const toolResult = await pool.query(`SELECT is_enabled FROM telemetry.tool_registry WHERE tool_name = $1`, [toolName]);

 ...
 */
export function createToolSpan(toolName, toolInput, parentSpanName = 'HTTP POST /ask') {
    const tracer = trace.getTracer('mcp-server');
 ...
 */
export function endToolSpan(span, result, error) {
    if (!span)
        return;
    if (error) {
        span.setStatus({
 ...
 */
export function recordContextSources(sources) {

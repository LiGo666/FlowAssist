...
export class ToolRegistry {
    constructor() {
        ...
        this.cacheValidityMs = 60000; // 1 minute
        this.pool = new Pool({
     ...
     */
    async canUseToolWithReason(toolName, userId, userRoles = ['user']) {
     ...
     */
    async canUseTool(toolName, userId, userRoles = ['user']) {
     ...
     */
    async getUserToolPreference(userId, toolName) {
        try {
            const result = await this.pool.query(`SELECT is_enabled FROM telemetry.user_tool_prefs 
     ...
     */
    async checkRateLimit(userId, toolName, period) {
        // Get tool from registry
        const tool = await this.getTool(toolName);
        if (!tool) {
    ...
    cleanupUsageCounters() {
        ...
        for (const [userKey, counters] of Array.from(this.usageCounters.entries())) {
            ...
            for (const bucket of counters.minute.keys()) {
                ...
                const bucketDate = new Date(parseInt(bucketParts[0]), parseInt(bucketParts[1]), parseInt(bucketParts[2]), parseInt(bucketParts[3]), parseInt(bucketParts[4]));

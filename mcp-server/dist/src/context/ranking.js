...
export class ContextRanker {
     ...
     */
    async rankContextSources(query, userId, options = {}) {
        ...
        try {
            ...
            const recentContext = recentContextValues
                .filter(Boolean)
            ...
            const similarContext = vectorSearchResults.map(result => ({
                id: result.id,
            ...
            const uniqueContext = Array.from(new Map(allContext.map(item => [item.id, item])).values());
            ...
            uniqueContext.sort((a, b) => (b.metadata.combinedScore || 0) - (a.metadata.combinedScore || 0));
            // Return top results
     ...
     */
    async storeContext(context, ttlSeconds = 3600) {
        try {
